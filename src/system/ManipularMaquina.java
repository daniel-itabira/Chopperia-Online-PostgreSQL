...
public class ManipularMaquina extends JFrame {
	...
	public void mostrarListaEstoque() {
		...
		try {
			...
			while (rs.next()) {
				ls.addElement(rs.getString("tipo") + "  " + rs.getString("marca") + " - " + df.format(rs.getDouble("quantidade"))
						+ " L");
			}
			...
			while (rs.next()) {
				ls.addElement(
						"Numero do Cartão: " + rs.getInt("nrCartao") + "  Número do pedido: " + rs.getInt("idPedido")
								...
								+ "  Produto: " + rs.getString("compra") + "  Valor: " + df.format(rs.getDouble("valor")));
			}
	...
	public void valorArrecadado() {
		...
		try {
			...
			labelArrecadado.setText("" + df.format(soma));
